<x-app-layout>
    {{-- 1. LOAD SWEETALERT DI SINI BIAR JALAN --}}
    @push('styles')
                            ...
                            @if($event->status == 'published' && $event->quota > 0 && now() < $event->end_datetime)
                                @auth
                                    <button
                                        ...
                                        class="w-full bg-indigo-600 text-white py-3 px-4 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200">
                                        Daftar Sekarang
                                    </button>
                                @endauth
...
<script>
    function showLoginAlert() {
        Swal.fire({
            ...
            text: "Kamu harus login dulu buat daftar event ini.",
            icon: 'warning',
