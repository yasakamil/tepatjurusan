                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="rounded-2xl overflow-hidden shadow-sm bg-gray-100">
                         @if($event->banner_media)
                            <img src="{{ Storage::url($event->banner_media) }}" alt="{{ $event->title }}" class="w-full h-auto object-cover">
                        @else
                            <div class="h-64 flex items-center justify-center text-gray-400">No Banner Available</div>
                        @endif
                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 sticky top-6">
                        ...
                        <div class="flex items-start mb-4">
                            <svg class="w-5 h-5 text-indigo-500 mt-1 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            <div>
                        ...
                        <div class="flex items-start mb-6">
                            <svg class="w-5 h-5 text-indigo-500 mt-1 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                            <div>
                                <p class="text-sm font-medium text-gray-900">Lokasi ({{ ucfirst($event->location_type) }})</p>
                            ...
                            @else
                                @if($event->discount_price && now() < $event->discount_end_time)
                                    <div class="flex items-center gap-2">
                                        <span class="text-gray-400 line-through text-sm">Rp {{ number_format($event->price, 0, ',', '.') }}</span>
                                        <span class="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded">Hemat {{ number_format((($event->price - $event->discount_price)/$event->price)*100) }}%</span>
                                    </div>
                                    <div class="text-2xl font-bold text-gray-900">Rp {{ number_format($event->discount_price, 0, ',', '.') }}</div>
                                @else
                                    <div class="text-2xl font-bold text-gray-900">Rp {{ number_format($event->price, 0, ',', '.') }}</div>
                                @endif
                            @endif
                        </div>
